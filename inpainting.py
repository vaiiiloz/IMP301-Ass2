...
class Inpainter():
    def __init__(self, image, mask, patch_size = 9, plot_progress = False):
        self.image = image.round().astype('uint8')
    ...
    def _plot_image(self):
        ...
        cv2.imwrite('results\\'+str(current_index)+'.png', image)
        # plt.clf()
        # plt.imshow(image)
        # plt.draw()
        # plt.pause(0.001)
    ...
    def _initialize_attributes(self):
        """Initialize the non initialized attibutes
        The confidence is initially the inverse of the mask

        The data starts with zero for all pixels

        The working image and working mask start as copies of the original image and mask.
        """
    ...
    def _find_front(self):
        """Find the front sing laplacian on the mask
        
        THe laplacian will give us the edges of the mask, it will be positive at the higer region (white)
        and negative at the lower region(black).
        We only want the white region, which is inside the mask, so we filter the negative values.
        """
