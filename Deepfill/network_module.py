...
import Deepfill.utils as utils
...
class TransposeConv2dLayer(nn.Module):
    ...
    def forward(self, x):
        x = F.interpolate(x, scale_factor = self.scale_factor, mode = 'nearest', recompute_scale_factor=True)
        x = self.conv2d(x)
        return x
...
class ContextualAttention(nn.Module):
    ...
    def forward(self, f, b, mask=None):
        ...
        # process mask
        mask = F.interpolate(mask, scale_factor=1./self.rate, mode='nearest', recompute_scale_factor=True)
        ...
        for xi, wi, raw_wi in zip(f_groups, w_groups, raw_w_groups):
            ...
            offset = torch.cat([torch.div(offset, int_fs[3], rounding_mode='floor'), offset%int_fs[3]], dim=1)  # 1*2*H*W
            # torch.div(offset, int_fs[3], rounding_mode='floor')
